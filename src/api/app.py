...
async def lifespan(fastapi_app: FastAPI):
    ...
    agent = await client.agents.get_agent(agent_id=os.getenv("AGENT_ID_ORCHESTRATOR"))
    # print(f"Agent retrieved: {agent}")
    fastapi_app.state.orchestrator_agent = AzureAIAgent(
        client=client, definition=agent, plugins=[ChatWithDataPlugin()]
    )
    yield
...
def build_app() -> FastAPI:
    ...
    async def get_config():
        ...
        config = {
            "API_URL": os.getenv(
                "REACT_APP_API_BASE_URL", os.getenv("API_URL", "http://127.0.0.1:8000")
            ),
            "CHAT_LANDING_TEXT": os.getenv(
